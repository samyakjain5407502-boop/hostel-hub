...
export function LanguageSwitcher({ className }: { className?: string }) {
  ...
  return (
    <RadixDropdown.Root>
      <RadixDropdown.Trigger className={cn('inline-flex h-10 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 shadow-lift hover:border-brand-300 hover:bg-brand-50', className)} aria-label="Change language">
        <Languages className="h-4 w-4 shrink-0 text-slate-500" aria-hidden="true" />
        <span className="min-w-[64px] text-left">{current.flag} {current.label}</span>
      </RadixDropdown.Trigger>
      <RadixDropdown.Portal>
        <RadixDropdown.Content
          sideOffset={8}
          align="end"
          className="z-[60] w-44 rounded-xl border border-slate-200 bg-white p-1.5 shadow-soft"
        >
